...
def mid(sess, op):
    ...
    for b in range(1, n_batches + 1):
        ...
        #print(batch_x[0])
        mid_lat = sess.run(op, feed_dict={x_input: batch_x})
        if (b == 1):
            mid_lat_all = mid_lat
        else:
            mid_lat_all = np.vstack((mid_lat_all, mid_lat))
    print(mid_lat_all)
    k = 2
    centroids, clusterAssment = KMeans1(mid_lat_all[:300,], k)
    # print(pred2[100:])
    showCluster(mid_lat_all[:300,], k, centroids, clusterAssment)
    ...
    with tf.Session() as sess:
        if train_model:
            ...
            for i in range(n_epochs):
                ...
                n_batches1 = int(mnist.train.num_examples / batch_size)
                for b in range(1, n_batches1 + 1):
                    batch_x, _ = mnist.train.next_batch(batch_size)
                    mid_lat = sess.run(encoder_output, feed_dict={x_input: batch_x})
                    if (b == 1):
                        mid_lat_all = mid_lat
                    else:
                        mid_lat_all = np.vstack((mid_lat_all[:400,], mid_lat))
                k = 2
                centroids, clusterAssment = KMeans1(mid_lat_all[:400,], k)
                showCluster(mid_lat_all[:400,], k, centroids, clusterAssment)
