...
def file_array():
    ...
    trainfile=np.concatenate((trainfile2, trainfile), axis=0)
...
X_SCdata1 = np.concatenate((X_train1, scdata1), axis=0)#源数据和生成数据结合（带东西），带标签
X_SCdata2 = np.concatenate((X_train2, scdata2), axis=0)#源数据和生成数据结合（不带东西），带标签
...
classer.save_weights('models/fido3_lat10-64/classer.h5')
ed.save_weights('models/fido3_lat10-64/ed.h5')
dd.save_weights('models/fido3_lat10-64/dd.h5')
sc_fido.save_weights('models/fido3_lat10-64/sc_fido.h5')