...
def other_file_arraymid():
    ...
    trainfile = trainfile[np.argsort(k)]
    trainfile = trainfile[:15]
    trainall1=np.concatenate((trainall1, trainfile), axis=0)
    trainfile=trainall1
    ...
    trainfile2 = trainfile2[np.argsort(k)]
    trainfile2 = trainfile2[:15]
    trainall2 = np.concatenate((trainall2, trainfile2), axis=0)
    trainfile2 = trainall2
...
X_train1 =train_feature_cut[:75*(lincut2 - cut1 * 2)]
...
X_train2 =train_feature_cut[75*(lincut2 - cut1 * 2):]
...
X_SCdata1=X_train1
X_SCdata2=X_train2
X_SCdata1_label=train_label_cut[:75*(lincut2 - cut1 * 2)]
X_SCdata2_label=train_label_cut[75*(lincut2 - cut1 * 2):]
X_SCdata1_domain_label=train_domain_label_cut[:75*(lincut2 - cut1 * 2)]
X_SCdata2_domain_label=train_domain_label_cut[75*(lincut2 - cut1 * 2):]
