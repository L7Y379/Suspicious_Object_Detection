...
def file_array3():
    ...
    trainfile = trainfile[np.argsort(k)]
    trainfile = trainfile[:20]
    testfile = np.concatenate((trainfile[0:1], trainfile[16:20]), axis=0)
    ...
    trainfile = trainfile[np.argsort(k)]
    trainfile = trainfile[:20]
    testfile = np.concatenate((trainfile[8:12], trainfile[19:20]), axis=0)
    ...
    trainfile = trainfile[np.argsort(k)]
    trainfile = trainfile[:20]
    testfile = np.concatenate((trainfile[8:10], trainfile[17:20]), axis=0)
...
X_test1 =test_feature_cut[:30*(lincut2 - cut1 * 2)]
print(X_test1.shape)
...
X_test2 =test_feature_cut[30*(lincut2 - cut1 * 2):]
print(X_test2.shape)
...
classer.load_weights('models/fido3_lat10-64upclasser2+yuandata0-1-ycut15-upfile/_1578_72y73_68_100_80m61_59_86_73m65_59_93_73classer.h5')
ed.load_weights('models/fido3_lat10-64upclasser2+yuandata0-1-ycut15-upfile/_1578_72y73_68_100_80m61_59_86_73m65_59_93_73ed.h5')
...
non_mid = ed.predict(test_feature[:lin2 * 30])
non_mid = non_mid[:, :latent_dim]
non_pre = classer.predict(non_mid)
yes_mid = ed.predict(test_feature[lin2 * 30:])
yes_mid = yes_mid[:, :latent_dim]
yes_pre = classer.predict(yes_mid)
print(non_mid.shape)
print(yes_mid.shape)
