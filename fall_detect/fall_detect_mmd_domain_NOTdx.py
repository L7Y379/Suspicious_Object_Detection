...
class ClassClassfier(nn.Module):
    ...
    def forward(self, ipt):
        ...
        return class_out
class DomainClassfier(nn.Module):
    def __init__(self, domain_num):
        super(DomainClassfier, self).__init__()
        self.class_classifier = nn. Sequential(
          nn.Linear(1025, 256),
          nn.BatchNorm1d(256),
          nn.ReLU(),
...
def train_test():
    ...
    dirPath = "D:/my bad/Suspicious object detection/data/fall/notdx_cut300/1202_pre"
    dirPath2 = "D:/my bad/Suspicious object detection/data/fall/notdx_cut300/1119_pre"
    dirPath3 = "D:/my bad/Suspicious object detection/data/fall/notdx_cut300/1123_pre"
    dirPath4 = "D:/my bad/Suspicious object detection/data/fall/notdx_cut300/1203_pre"
    dirPath_test = "D:/my bad/Suspicious object detection/data/fall/notdx_cut300/1123_pre"
...
def train():
    ...
    domainClassfier = DomainClassfier(domain_num).to(device)
    ...
    optimizer_domainClassfier = optim.Adam(domainClassfier.parameters(), lr)
    loss_min=10000
    for epoch in range(1, 1 + epochs):
        ...
        source_item_doaminlabel = torch.from_numpy(train_label[idx]).to(device)
        source_item_doaminlabel = torch.argmax(source_item_doaminlabel, 1)
        optimizer_feature_extractor.zero_grad()
        optimizer_classClassfier.zero_grad()
        optimizer_domainClassfier.zero_grad()
        feature = feature_extractor(source_item_csi_data)
        feature_class, feature_domain = feature.split(1025, dim=1)  # 将维度0拆分成每个都是1的
        # print(feature_class.shape)
        # print(feature_domain.shape)
        class_out = classClassfier(feature_class)
        domain_out = domainClassfier(feature_domain)
        # print(feature.shape)
        # print(class_out.shape)

        # feature_extractor.train()
        # classClassfier.train()
        mmd = 1
        if (mmd == 1):
            ...
            feature_target_class, feature_target_domain = feature.split(1025, dim=1)
            mmd_loss = mmd_rbf(feature_class, feature_target_class)
            loss = loss_class(class_out, source_item_label) + mmd_loss * lambd + loss_class(domain_out,
                                                                                            source_item_doaminlabel)
        if (mmd == 0):
            loss = loss_class(class_out, source_item_label) + loss_class(domain_out, source_item_doaminlabel)
        loss.backward()
        optimizer_feature_extractor.step()
        optimizer_classClassfier.step()
        optimizer_domainClassfier.step()
        acc_num = 0
        if epoch % 10 == 0:
            feature = feature_extractor(source_item_csi_data)
            feature_class, feature_domain = feature.split(1025, dim=1)  # 将维度0拆分成每个都是1的
            ...
            acc_num += (pred == source_item_label).sum().item()
            acc = acc_num / source_item_label.shape[0]
            print("%d [fall_detection_loss: %f,acc: %.2f%%]" % (epoch, loss, 100 * acc))
            n = 0
            a_all = np.zeros((3, 2))  # 3个动作：第一个跌倒，后两个个非跌倒
            p = 0  # 每个人样本数，0表示20个，1表示40个
            q = 4  # 人数
            if (p == 0):
                for o in range(q):  # 四个人的数据
                    # print(test_feature.shape)
                    # feature_extractor.eval()
                    # classClassfier.eval()
                    non_mid = feature_extractor(torch.tensor(test_feature[o * 20:(o + 1) * 20], dtype=torch.float32).to(
                        device))  # 每个人20条数据，10条跌倒，10条非跌倒
                    non_pre, non_pre_domain = non_mid.split(1025, dim=1)
                    non_pre = classClassfier(non_pre)  # (20,2)
                    m = 0
                    a = np.zeros((3, 2))
                    for i in range(4):
                        for k in range(5):
            ...
            if (p == 1):
                for o in range(q):  # 四个人的数据
                    ...
                    # feature_extractor.eval()
                    # classClassfier.eval()
                    non_mid = feature_extractor(torch.tensor(test_feature[o * 40:(o + 1) * 40], dtype=torch.float32).to(
                        device))  # 每个人40条数据，20条跌倒，20条非跌倒
                    non_pre, non_pre_domain = non_mid.split(1025, dim=1)
                    non_pre = classClassfier(non_pre)  # (40,2)
                    m = 0
                    a = np.zeros((3, 2))
                    for i in range(4):
                        for k in range(10):
                            x = torch.argmax(non_pre[i * 10 + k])
                            if (i == 0 or i == 1):
                                a[0][x] = a[0][x] + 1
                                a_all[0][x] = a_all[0][x] + 1
                            else:
