...
def read_datamid_huodong():
    ...
    for i in range(54):
        if((i%6==4)or(i%6==5)):
            ...
            idx2 = np.array([j for j in range((i * 20 + 12)*200, (i * 20 + 20)*200, 1)])  # 取中心点处左右分布数据
            idx = np.hstack((idx1, idx2))
            idxt = np.array([j for j in range((i * 20 + 8)*200, (i * 20 + 12)*200, 1)])
            # print(filename)
            if i == 4:
            ...
            idx1 = np.array([j for j in range((i * 20) * 200, (i * 20 + 4) * 200, 1)])  # 取中心点处左右分布数据
            idx2 = np.array([j for j in range((i * 20 + 16) * 200, (i * 20 + 20) * 200, 1)])  # 取中心点处左右分布数据
            idx = np.hstack((idx1, idx2))
            idxt = np.array([j for j in range((i * 20 + 8) * 200, (i * 20 + 10) * 200, 1)])
            # print(filename)
            if i == 0:
                temp_feature3 = train_feature[idx]
                temp_feature4 = train_feature[idxt]
            else:
                temp_feature3 = np.concatenate((temp_feature3, train_feature[idx]), axis=0)
                temp_feature4 = np.concatenate((temp_feature4, train_feature[idxt]), axis=0)

    print(temp_feature.shape)
    print(temp_feature2.shape)
    print(temp_feature3.shape)
    print(temp_feature4.shape)
    train_feature=np.concatenate((temp_feature3, temp_feature), axis=0)
    test_feature = np.concatenate((temp_feature4, temp_feature2), axis=0)
    print(train_feature.shape)#(115200,90)
    print(test_feature.shape)#(28800,90)
