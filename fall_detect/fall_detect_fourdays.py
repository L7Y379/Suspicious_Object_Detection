...
def train_t(train_feature,test_feature,train_label,model_path):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0:
            ...
            if( epoch != 0 and acc_test<ac and acc < crnn_loss[1]):
                acc_test=ac
                acc = crnn_loss[1]
                crnn_model.save_weights(model_path)
...
def model_train_test4(dirname,dirname2,dirname3,dirname4,dirname5,dirname6,dirPath_test, model_path):
    k = 0 #标识符 判断数据列表是否新建
    print("进入模型训练。。。")
    ...
    dataList = os.listdir(dirname5)
    for i in range(0, len(dataList)):
        path = os.path.join(dirname5, dataList[i])
        if os.path.isfile(path):
            temp_data = pd.read_csv(path, error_bad_lines=False, header=None)
            temp_data = np.array(temp_data, dtype=np.float64)
            raw_data = np.row_stack((raw_data, temp_data))
    ...
    dataList = os.listdir(dirname6)
    for i in range(0, len(dataList)):
        path = os.path.join(dirname6, dataList[i])
        if os.path.isfile(path):
            temp_data = pd.read_csv(path, error_bad_lines=False, header=None)
            temp_data = np.array(temp_data, dtype=np.float64)
            raw_data = np.row_stack((raw_data, temp_data))
...
def train_test():
    ...
    model_path = "D:/my bad/Suspicious object detection/Suspicious_Object_Detection/yue/fall_detect/models/1112_1115_1117_1119.h5"
    dirPath="D:/my bad/Suspicious object detection/data/fall/1112_1115_1117_1119_pre/1112_pre"
    dirPath2 = "D:/my bad/Suspicious object detection/data/fall/1112_1115_1117_1119_pre/1115_pre"
    dirPath3 = "D:/my bad/Suspicious object detection/data/fall/1112_1115_1117_1119_pre/1117_pre"
    dirPath4 = "D:/my bad/Suspicious object detection/data/fall/1112_1115_1117_1119_pre/1119_pre"
    dirPath5 = "D:/my bad/Suspicious object detection/data/fall/1112_1115_1117_1119_pre/1112_pre_test"
    dirPath6 = "D:/my bad/Suspicious object detection/data/fall/1112_1115_1117_1119_pre/1119_test_pre"
    dirPath_test = "D:/my bad/Suspicious object detection/data/fall/1112_1115_1117_1119_pre_test"
    model_train_test4(dirPath, dirPath2,dirPath3,dirPath4,dirPath5,dirPath6, dirPath_test, model_path)
def test_walk():
    modelName = "D:/my bad/Suspicious object detection/Suspicious_Object_Detection/yue/fall_detect/models/1112_1115_1117_1119.h5"
    dirname = "D:/my bad/Suspicious object detection/data/fall/1112_1115_1117_1119_pre_test"
    k = 0  # 标识符 判断数据列表是否新建
    print("进入模型训练。。。")
    dataList = os.listdir(dirname)
    for i in range(0, len(dataList)):
