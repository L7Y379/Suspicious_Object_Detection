...
def build_cnn(img_shape):
    cnn = Sequential()
    cnn.add(TimeDistributed(Conv1D(8, kernel_size=10, activation='relu',padding='same',input_shape=img_shape)))
    cnn.add(TimeDistributed(MaxPooling1D(pool_size=4)))
    cnn.add(TimeDistributed(Conv1D(16, kernel_size=10,activation='relu', padding='same')))
    cnn.add(TimeDistributed(MaxPooling1D(pool_size=4)))
    cnn.add(TimeDistributed(Conv1D(32, kernel_size=10, activation='relu', padding='same')))
    cnn.add(TimeDistributed(MaxPooling1D(pool_size=4)))
...
def train_test():
    ...
    model_path = "D:/my bad/Suspicious object detection/Suspicious_Object_Detection/yue/fall_detect/models/1112_1115_1119_notdx_yiweicnn.h5"
    dirPath = "D:/my bad/Suspicious object detection/data/fall/notdx/1112_pre"
    dirPath2 = "D:/my bad/Suspicious object detection/data/fall/notdx/1115_pre"
    dirPath3 = "D:/my bad/Suspicious object detection/data/fall/notdx/1119_pre"
    dirPath_test = "D:/my bad/Suspicious object detection/data/fall/notdx/1117_pre"
...
def test_walk():
    modelName = "D:/my bad/Suspicious object detection/Suspicious_Object_Detection/yue/fall_detect/models/1112_1115_1119_notdx_yiweicnn.h5"
    dirname = "D:/my bad/Suspicious object detection/data/fall/notdx/1117_pre"
    k = 0  # 标识符 判断数据列表是否新建
    print("进入模型训练。。。")
    dataList = os.listdir(dirname)
    for i in range(0, len(dataList)):
